...
/** Strip `**phrase**` markers used for on-page emphasis (metadata must stay plain). */
function stripInlineEmphasisMarkers(text: string): string {
  return text.replace(/\*\*([^*]+)\*\*/g, "$1");
}

/** Renders optional `**highlight**` spans as orange bold + purple underline. */
function formatDanceEmphasis(text: string): ReactNode {
  ...
  return parts.map((part, i) => {
    if (part.startsWith("**") && part.endsWith("**") && part.length > 4) {
      return (
        <strong
          key={i}
          className="font-semibold text-[#F28904] underline decoration-[#200073] decoration-2 underline-offset-[0.2em]"
        >
          {part.slice(2, -2)}
        </strong>
      );
...
interface BlogPost {
  ...
  /** Optional third body paragraph (e.g. Apsara). */
  contentThird?: string
  thumbnail: string
  thumbnails: string[]
  labels: string[]
  ...
  private posts: BlogPost[] = [
    {
      ...
      title: "Apsara",
      contentFirst:
        "Apsara is a **classical Cambodian dance** that draws inspiration from the celestial dancers, or 'Apsaras,' in Hindu mythology, who are believed to be divine and graceful beings. The dance is characterized by slow, fluid movements that emphasize elegance and beauty.",
      contentSecond:
        "Dancers wear intricate costumes, including headdresses and jewelry, and perform to music from the traditional **Pinpeat orchestra**. The dance embodies divine grace and is often performed to honor the gods or during important cultural ceremonies and celebrations.",
      contentThird:
        "The Apsara dance has deep spiritual and cultural significance in Cambodia, often seen as a **connection between the human and divine realms**. The dancers convey complex emotions and stories through intricate hand gestures and delicate poses, bringing to life the celestial beings they represent. The Apsara dance is considered a symbol of **grace and beauty**, often performed during royal events, national celebrations, and religious ceremonies. It has become an iconic part of Cambodian culture and heritage, showcasing the country's rich artistic history.",
...
export async function generateMetadata({
  ...
  const description = metaDescription(
    stripInlineEmphasisMarkers(
      [post.contentFirst, post.contentSecond, post.contentThird].filter(Boolean).join(" ")
    )
  );
...
export default function BlogPostPage({
  ...
  const allRepertoire = [...CLASSICAL_DANCES, ...FOLK_DANCES]
  const otherPerformances = allRepertoire.filter((d) => d.href !== `/dances/${post.slug}`)
      ...
      <div className="bg-white">
        <div className="flex justify-center w-full p-5 pt-0 mb-10 px-5 lg:px-10 animate-[fade-me-in_.5s_ease-in-out] bg-white">
          <div className="flex flex-col w-full max-w-[1040px] mt-[130px] mb-5">
              ...
              <div className="text-md">
                ...
                >
                  ...
                  <div className="font-light text-md">
                    {formatDanceEmphasis(post.contentFirst)}
                    <br /><br />
                    {formatDanceEmphasis(post.contentSecond)}
                    {post.contentThird ? (
                      <>
                        <br /><br />
                        {formatDanceEmphasis(post.contentThird)}
                      </>
                    ) : null}
            ...
            <section className="scroll-mt-28" aria-labelledby="other-performances-heading">
              <h2
                id="other-performances-heading"
                className="border-b-2 border-[#F28904] pb-3 text-center font-serif text-2xl font-bold text-[#1a1a1a] sm:text-left"
              >
                Other performances
              </h2>
              <RepertoireDanceGrid
                dances={otherPerformances}
                className="mt-6 sm:mt-8"
                ariaLabel="Other performances"
              />
            </section>
